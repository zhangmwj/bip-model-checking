...
test_or_expressions() {
  ...
  $OUTPUT_DIR/build/system > $OUTPUT_DIR/trace || fail "Executable returned an error"
  INTER_NB=$(grep '1 interaction' -c $OUTPUT_DIR/trace)
  INTER_NB=$(grep 'choose.*single.*synchro' -c $OUTPUT_DIR/trace)
...
# priority_rules_cycle4.bip
test_priority_rules_cycle4() {
  TEST=priority_rules_cycle4
  compute_output_dir
...
# priority_rules_cycle5.bip
test_priority_rules_cycle5() {
  TEST=priority_rules_cycle5
  compute_output_dir

  compile_to_cpp "$TEST" "$RUNNING_TESTS" 'Level1()' && compile_and_link_with_engine "$TEST" || fail "Compiler returned an error"
...
# priority_rules_no_cycle3.bip
test_priority_rules_no_cycle3() {
  TEST=priority_rules_no_cycle3
  compute_output_dir
  ...
  grep -q 'state #0: 1 interaction' $OUTPUT_DIR/trace && \
  grep -q 'brd:.*A1.p.*A2.p.*A3.p' $OUTPUT_DIR/trace
}


# priority_rules_no_cycle4.bip
test_priority_rules_no_cycle4() {
  TEST=priority_rules_no_cycle4
  compute_output_dir

  compile_to_cpp "$TEST" "$RUNNING_TESTS" 'Compound()' && compile_and_link_with_engine "$TEST" || fail "Compiler returned an error"

  $OUTPUT_DIR/build/system --silent
  assertEquals 0 $? # returns no error
}
...
test_broadcast_browser() {
  ...
  NBsync=$(grep -m 63 -e 'choose.*synchro' -c $OUTPUT_DIR/trace)
  NBstop=$(grep -m2 -e 'choose.*stop' -c $OUTPUT_DIR/trace)

  NBsyncbit1=$(grep -m 32 -e 'choose.*synchro.*bit1' -c $OUTPUT_DIR/trace)
  NBsyncbit2=$(grep -m 32 -e 'choose.*synchro.*bit2' -c $OUTPUT_DIR/trace)
  NBsyncbit3=$(grep -m 32 -e 'choose.*synchro.*bit3' -c $OUTPUT_DIR/trace)
  NBsyncbit4=$(grep -m 32 -e 'choose.*synchro.*bit4' -c $OUTPUT_DIR/trace)
  NBsyncbit5=$(grep -m 32 -e 'choose.*synchro.*bit5' -c $OUTPUT_DIR/trace)
  NBsyncbit6=$(grep -m 32 -e 'choose.*synchro.*bit6' -c $OUTPUT_DIR/trace)
...
test_maximal_progress() {
  ...
  (grep -q 'choose.*high:.*low.xport(x=-5;)'  $OUTPUT_DIR/trace && grep -q 'single:.*myCompound.p(x=-12;)' $OUTPUT_DIR/trace) ||
  (grep -q 'choose.*high:.*low.xport(x=-12;)' $OUTPUT_DIR/trace && grep -q 'single:.*myCompound.p(x=-5;)'  $OUTPUT_DIR/trace) ||
  (grep -q 'single:.*myCompound.p(x=-5;)'     $OUTPUT_DIR/trace && grep -q 'single:.*myCompound.p(x=-12;)' $OUTPUT_DIR/trace)
...
#  TEST=doublecounter
#  compute_output_dir
#  compile_to_cpp "$TEST" "$RUNNING_TESTS" 'CT()' "--gencpp-enable-marshalling" && compile_and_link_with_engine "$TEST" || fail "Compiler returned an error"

#  $OUTPUT_DIR/build/system --explore > $OUTPUT_DIR/trace || fail "Executable returned an error"

#  declare -i success

#  let "success+=$(grep "10201 reachable states" -c $OUTPUT_DIR/trace)"
#  let "success+=$(grep "1 deadlock" -c $OUTPUT_DIR/trace)"
#  let "success+=$(grep "0 error" -c $OUTPUT_DIR/trace)"
...
#  let "success+=$(grep "5251 reachable states" -c $OUTPUT_DIR/trace)"
#  let "success+=$(grep "1 deadlock" -c $OUTPUT_DIR/trace)"
#  let "success+=$(grep "0 error" -c $OUTPUT_DIR/trace)"
...
#  let "success+=$(grep "251001 reachable states" -c $OUTPUT_DIR/trace)"
#  let "success+=$(grep "0 deadlock" -c $OUTPUT_DIR/trace)"
#  let "success+=$(grep "0 error" -c $OUTPUT_DIR/trace)"
...
#  TEST=dining_philo
#  compute_output_dir
#  compile_to_cpp "$TEST" "$RUNNING_TESTS" 'DiningPhilo()' "--gencpp-enable-marshalling" && compile_and_link_with_engine "$TEST" || fail "Compiler returned an error"
...
#  let "success+=$(grep "117485 reachable states" -c $OUTPUT_DIR/trace)"
#  let "success+=$(grep "2 deadlock" -c $OUTPUT_DIR/trace)"
#  let "success+=$(grep "0 error" -c $OUTPUT_DIR/trace)"
...
# dining_philo_flat.bip
#test_dining_philo_flat() {
#  TEST=dining_philo_flat
#  compute_output_dir
#  compile_to_cpp "$TEST" "$RUNNING_TESTS" 'DiningPhilo()' "--gencpp-enable-marshalling" && compile_and_link_with_engine "$TEST" || fail "Compiler returned an error"
