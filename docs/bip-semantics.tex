...
\begin{itemize}
  ...
  \item $\xrightarrow{}_A$ is the transition relation defined by the rules:
        \begin{mathpar}
          \inferrule[port]{
            \langle l, p, g, f, l', u \rangle \in T_A \\
            v \vDash g \\
            v' = f(v/v_p)~(1)\\
            v' \vDash I_A(l')
          } {
...
\begin{itemize}
  ...
  \item $\Sigma_C = \mathbb{R}^{+*} \cup \bigl(\bigcup_{\gamma \in \Gamma_C} \mathcal{A}(\gamma) \cup \mathcal{B}(C)
          \bigr) \times \{e,l\}$,
  \item $\xrightarrow{}_C$ is defined by the rules:
        ...
        \begin{mathpar}
          \inferrule[interaction]{
            ...
            v \vDash g \\
            \forall i \in I, \exists u_i \in \{e,l\}: q_i \xrightarrow{\langle p_i, v_i', u_i \rangle}_{A_i} q_i'
            \wedge v_i' = f \vert_{x_{p_i}}(v)~(3) \\
            u = \text{$\begin{cases}
